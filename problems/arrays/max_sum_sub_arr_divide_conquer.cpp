...
int maxSumSubArray( vector<int> &A, int l, int h ){
    if( l == h )
        return A[l];

    int m = (l + h)/2;

    int left_mss = maxSumSubArray( A, l, m );
    int right_mss = maxSumSubArray( A, m+1, h );

    int left_cross_max_sum = numeric_limits<int>::min();
    int right_cross_max_sum = numeric_limits<int>::min();
    int sum = 0;

    for( int i=m; i>= 0; i-- ){
        sum += A[i];
        if( sum > left_cross_max_sum )
            left_cross_max_sum = sum;
    }

    sum = 0;
    for( int i=m+1; i<=h; i++ ){
        sum += A[i];
        if( sum > right_cross_max_sum )
            right_cross_max_sum = sum;
    }

    return max( max( left_mss, right_mss ), left_cross_max_sum + right_cross_max_sum );
}
...
int main() {
    ...
    int max_sum = maxSumSubArray(A, 0, (int) (A.size() - 1));
